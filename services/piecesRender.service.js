...
export const piecesRender = {
    ...
    // рендеринг доски и шахматных фигур
    renderPieces() {
        // информация об изначальной позиции фигур на доске
        const gameSetup = chessConfig.useInitialGame ? initialGame : potentialGame

        this.placePieceBoxNumbers()  // рендеринг наименований позиций на доске через их id-шники, можно выключить как тут, так и через css display: none
        this.placeWhiteDownOrUp()
        this.placePiecesInPosition( gameSetup )  // позиции фигур на доске
    ...
    placePieceBoxNumbers() {
        $$( chessConfig.chessPieceBoxSelector ).map( pieceBoxElement => {
            ...
            // вытаскиваем из каждого div элемента значение внутри id и помещаем в span
    ...
    placeWhiteDownOrUp() {
        // через флексы мы управляем положением белых и черных фигур на доске перемещая вниз или вверх
